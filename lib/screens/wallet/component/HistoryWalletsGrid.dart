...
class HistoryWalletsGrid extends StatefulWidget {
  const HistoryWalletsGrid({Key key}) : super(key: key);

  @override
  _HistoryWalletsGridState createState() => _HistoryWalletsGridState();
}

class _HistoryWalletsGridState extends State<HistoryWalletsGrid> {
...
class DessertDataSource extends DataTableSource {
  ...
  DataRow getRow(int index) {
    ...
    return DataRow.byIndex(index: index, cells: <DataCell>[
      ...
      DataCell(Text(
          (dessert.type == "0" ? "Pay" : "Recharge ") + " €" + dessert.amount)),
      // DataCell(Text(dessert.amount,
      //     style: TextStyle(color: Colors.red, backgroundColor: Colors.yellow))),
      DataCell(Text(
        ((dessert.listingId == "" && dessert.type == "0")
            ? "Renew Package"
            : dessert.listingTitle),
